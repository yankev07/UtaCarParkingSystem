...
//		  request.setAttribute("errorMessage13", "User Name Cannot be Empty");
//		  pass = false;
//	  }
	  if(!errorHandler.checkUserNameLength(data.getUserName())) {
		  request.setAttribute("errorMessage13", "Username Format Error");
		  pass = false;
	  }
	  ...
	  if(!errorHandler.checkPhoneLength(data.getPhone())) {
		  request.setAttribute("errorMessage8", "Phone Lenght Error");
		  pass = false;
	  }
	  ...
	  if(pass){
		  ...
		  if(dataObject.updateAdminProfile(data.getUserName(), data.getPassword(), data.getPhone(), data.getEmail(), data.getAddress(), data.getZipCode())) {
			  response.sendRedirect("updateAdminProfileSuccess.jsp");
		  }
		  else {
